...
$id = $_GET['id'];
$query = "SELECT * FROM slot_waktu_221061 WHERE id_slot_221061 = $id";
$result = mysqli_query($koneksi, $query);
$data = mysqli_fetch_assoc($result);


if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $id = $_POST['id_slot_221061'];
    ...
    if($minutes != "00") {
        echo "<script>
                ...
                window.location.href='edit_slot.php?id=" . $id . "';
              </script>";
        exit;
    }
    
    // Cek apakah jam sudah ada di database (kecuali untuk record yang sedang diedit)
    $check_query = "SELECT * FROM slot_waktu_221061 WHERE jam_221061 = '$jam' AND id_slot_221061 != '$id'";
    ...
    $query = "UPDATE slot_waktu_221061 
              SET jam_221061 = '$jam', 
                  status_221061 = '$status'
              WHERE id_slot_221061 = '$id'";
    
    if (mysqli_query($koneksi, $query)) {
        echo "<script>
                alert('Slot waktu berhasil diupdate');
          ...
          <div class="content-wrapper">
            ...
            <div class="row">
              <div class="col-md-6 grid-margin stretch-card">
                ...
                <div class="card-body">
                     <form method="POST">
                                        <input type="hidden" name="id_slot_221061" value="<?= $data['id_slot_221061'] ?>">
                                        <div class="mb-3">
                                            <label for="jam" class="form-label">Jam</label>
                                            <input type="time" class="form-control" id="jam" name="jam_221061" step="3600" value="<?= $data['jam_221061'] ?>" required>
                                        </div>
                                        <div class="mb-3">
                                            <label for="status" class="form-label">Status</label>
                                            <select class="form-select" id="status" name="status_221061">
                                                <option value="Aktif" <?= $data['status_221061'] == 'Aktif' ? 'selected' : '' ?>>Aktif</option>
                                                <option value="Nonaktif" <?= $data['status_221061'] == 'Nonaktif' ? 'selected' : '' ?>>Nonaktif</option>
                                            </select>
                                        </div>
                                        <button type="submit" class="btn btn-primary">Update</button>
                                        <a href="slotwaktu.php" class="btn btn-light">Kembali</a>
                                    </form>
                </div>
                </div>
              </div>
